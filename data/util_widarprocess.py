...
def upsample(signal,time_stamp,newtimelen):
    ...
    while new_time_index<newtimelen:
        ...
        new_time_index+=1
    new_time_stamp = np.clip(new_time_stamp, time_stamp[0], time_stamp[-1]) #zzy加的 防止报错
...
def update_widar_paths(input_txt_path, old_prefix, new_prefix, save_path=None):
    updated_lines = []
    ...
    for line in lines:
        ...
        old_path, mapped_name = line.split('->')
        ...
        if old_path.startswith(old_prefix):
            ...
            new_path = old_path  # 不符合格式的保持原样

        updated_line = f"{new_path} -> {mapped_name}"
...
def convert_npy_to_mat(input_dir, output_dir, var_name='data'):
    """
    将 input_dir 中的所有 .npy 文件转换成 .mat 文件，
    并保存到 output_dir 下。

    参数:
    - input_dir: 输入文件夹路径 (包含 .npy 文件)
    - output_dir: 输出文件夹路径 (存放 .mat 文件)
    - var_name: .mat 文件中数据的变量名 (默认: 'data')
    """
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.npy'):
                ...
                mat_file = os.path.join(output_dir, base_name + '.mat')
...
if __name__ == '__main__':
    # input_path = '/opt/data/private/FDAARC/data/widar_stat/output.txt'
    # old_prefix = '/data0/wifi_datasets/datasets/widar/'
    # new_prefix = '/opt/data/common/default/wifidata/widar/'
    # output_path = '/opt/data/private/FDAARC/data/widar_stat/updated_output.txt'

    # updated = update_widar_paths(input_path, old_prefix, new_prefix, save_path=output_path)

    # # 可选：打印前几行查看结果
    # for line in updated[:5]:
    #     print(line)
    convert_npy_to_mat(
    input_dir='/opt/data/private/ablation_study/data_widar_800/tdcsi/',
    output_dir='/opt/data/private/ablation_study/data_widar_mat/data_widar_tdcsi_mat/'
)
